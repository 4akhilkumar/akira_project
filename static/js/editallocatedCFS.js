...
$('.checkbox').on('mousemove mouseenter mouseover', function(e) {
    var courseListTop = $('.courses-list').offset().top;
    var checkboxTop = $(this).offset().top;
    if(checkboxTop - courseListTop > 120) {
        $(this).removeClass('vtoptooltip');
        $(this).addClass('vbottomtooltip');
        $(this).find('.tooltiptext').css('bottom', 'unset');
        $(this).find('.tooltiptext').css('top', '-11px');
        $(this).find('.tooltiptext').css('transform', 'translate(-50%, -100%)');
    } else {
        $(this).removeClass('vbottomtooltip');
        $(this).addClass('vtoptooltip');
        $(this).find('.tooltiptext').css('top', 'unset');
        $(this).find('.tooltiptext').css('bottom', '-11px');
        $(this).find('.tooltiptext').css('transform', 'translate(-50%, 100%)');
    }
});

$("#back_allocatedcfs").click(function () {
    var backToallocatedcfs = $(this).data('back-to-allocated-cfs-url');
    $("#back_allocatedcfs").prop("disabled", true);
    setTimeout(function () {
        $("#back_allocatedcfs").prop("disabled", false);
    }, 5000);
    window.location.href = backToallocatedcfs;
});